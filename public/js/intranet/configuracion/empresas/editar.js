    ...
    $.ajax({
        ...
        success: function(respuesta) {
            if (respuesta.mensaje == "Activada") {
                ...
                $("#botonEstado_id").html('Empresa Activa');

                Sistema.notificaciones('La empresa se activo de manera correcta', 'Sistema', 'success');
            } else {
                $('#botonEstado_id').removeClass('btn-success');
                $('#botonEstado_id').addClass('btn-secondary');
                $('#botonEstado_id').attr('data_estado',0);
                $("#botonEstado_id").html('Empresa Inactiva');

                Sistema.notificaciones('La empresa se inactivo de manera correcta', 'Sistema', 'warning');
            }
